...
//				});
        ...
        On.get(ENDPOINT_BYTE_BUFFER).managed(false).html((req, resp) -> {
            String type = req.header(ACCEPT_HEADER);
            if (type == null || type.equalsIgnoreCase(MimeType.JSON.toString())) {
    ...
    public static void startSparkServer() {
        ...
        get("/hello.html", MimeType.HTML.toString(), (req, res) -> "<html><header><title>Spark (HTML-style): Hello World!</title></header><body><h1>Hello world!</h1></body></html>");
        get(ENDPOINT_HELLO, MimeType.HTML.toString(), (req, res) -> "<html><header><title>Spark (HTML-style): Hello World!</title></header><body><h1>Hello world!</h1></body></html>");
        get(ENDPOINT_HELLO, MimeType.JSON.toString(), (req, res) -> SELF.new MyData("Spark (JSON-style): Hello World!"), gson::toJson);
        ...
        get(ENDPOINT_BYTE_BUFFER, MimeType.JSON.toString(), (req, res) -> new MyBinaryData(BYTE_BUFFER), JSON::toJSONString);
...
//		});		
        ...
        HANDLER_TCP_SERVER.setServiceHandler((key, channel) -> {
            ...
            if (operation.toLowerCase(Locale.UK).contains("byte")) {
                ...
                if (numWrite != BYTE_BUFFER.length) {
                    LOGGER.atError().addArgument(numWrite).addArgument(BYTE_BUFFER.length).log("write mismatch: wrote {} bytes should have {} bytes");
                }
            } else {
    ...
    public enum MimeType {
        PLAINTEXT("text/plain"), BINARY("application/octet-stream"), JSON("application/json"), XML("text/xml"), HTML("application/html"), UNKNOWN("application/octet-stream");
    ...
    @SpringBootApplication
    @EnableAutoConfiguration(exclude = EmbeddedWebServerFactoryCustomizerAutoConfiguration.class)
    @Import({ WebFluxHelloRouter.class, WebFluxByteBufferRouter.class, WebFluxHelloHandler.class, WebFluxByteBufferHandler.class })
    ...
    public class WebFluxByteBufferHandler {
        public Mono<ServerResponse> byteArray(ServerRequest request) {
            ...
            if (acceptedMediaTypes.contains(org.springframework.http.MediaType.APPLICATION_JSON)) {
                // return ServerResponse.ok().contentType(org.springframework.http.MediaType.APPLICATION_JSON).body(Mono.just(gson.toJson(new MyBinaryData(buffer))), String.class);
                return ServerResponse.ok().contentType(org.springframework.http.MediaType.APPLICATION_JSON).body(Mono.just(JSON.toJSONString(new MyBinaryData(BYTE_BUFFER))), String.class);
                ...
                return ServerResponse.ok().contentType(org.springframework.http.MediaType.APPLICATION_OCTET_STREAM).body(BodyInserters.fromDataBuffers(Flux.just(dataBuffer)));
            }
            // default return is raw buffer
            DefaultDataBuffer dataBuffer = FACTORY.wrap(ByteBuffer.wrap(BYTE_BUFFER));
            return ServerResponse.ok().contentType(org.springframework.http.MediaType.IMAGE_JPEG).body(BodyInserters.fromDataBuffers(Flux.just(dataBuffer)));
        }
    }
    ...
    public class WebFluxHelloHandler {
        public Mono<ServerResponse> hello(ServerRequest request) {
            return ServerResponse.ok().contentType(org.springframework.http.MediaType.TEXT_PLAIN).body(BodyInserters.fromObject("Spring: Hello World!"));
        }
    }
    ...
    public class WebFluxHelloRouter {
        ...
        public RouterFunction<ServerResponse> helloRoute(WebFluxHelloHandler handler) {
            return RouterFunctions.route(GET(ENDPOINT_HELLO).and(accept(org.springframework.http.MediaType.TEXT_PLAIN)), handler::hello);
        }
    }
